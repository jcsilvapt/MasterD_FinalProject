...
public class Fabio_EnemySecondLevel : MonoBehaviour, AIStateMachine, IDamage
{
    ...
    public void CheckSurroundingEnemies() //see if there are enemies nearby and activate behaviours to help them attack the player
    {
        if (allyAlert == true)
        {
            ...
            foreach (Collider Ally in hitColliders)
            {
                ...
                {
                    if (Ally.transform.GetComponent<Fabio_EnemySecondLevel>().currentState != AIStates.GotHit || Ally.transform.GetComponent<Fabio_EnemySecondLevel>().currentState != AIStates.Chase )
                    {
                        Ally.transform.GetComponent<Fabio_EnemySecondLevel>().HandleEvent(AIEvents.GotAttacked);
                    }
                }
            }
        }
